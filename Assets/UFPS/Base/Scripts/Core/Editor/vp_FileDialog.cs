...
public class vp_FileDialog : EditorWindow
{
	...
	void OnGUI()
	{
		...
		// --- file area ---
		GUILayout.BeginArea(new Rect(20, 20, Screen.width / 2 - 40 , Screen.height / 2 - 40));

		// --- path field ---
		if (Path == Application.dataPath)
			GUI.enabled = false;
		Vector2 pathPixelSize = m_PathFieldLeftAlignStyle.CalcSize(new GUIContent(Path));
		GUIStyle pathFieldStyle = (pathPixelSize.x > Screen.width / 2 - 66) ?
									m_PathFieldRightAlignStyle : m_PathFieldLeftAlignStyle;
		GUI.TextField(new Rect(0, 0, Screen.width / 2 - 66, 20), Path, pathFieldStyle);

		// --- up button ---
		if (GUI.Button(new Rect(Screen.width / 2 - 62, 0, 22, 20), "^", m_UpButtonFileStyle))
			GoUpOneDir();
		GUI.enabled = true;

		// --- file box ---
		float height = m_Mode == Mode.Open ? Screen.height / 2 - 106 : Screen.height / 2 - 130;
		FileBox(0, 24, Screen.width / 2 - 20, height);
		...
		Vector2 pos = m_Mode == Mode.Open ?
			new Vector2(20, Screen.height / 2 - 58) : new Vector2(20, Screen.height / 2 - 82);
		GUILayout.BeginArea(new Rect(pos.x, pos.y, Screen.width / 2 - 40, Screen.height / 2 - 40));
		...
		if (m_Mode == Mode.Save)
		{
			...
			if(m_SelectedItemType == SelectedItemType.Dir)
				GUI.TextField(new Rect(0, 0, Screen.width / 2 - 124, 20), "");
			else
				Filename = GUI.TextField(new Rect(0, 0, Screen.width / 2 - 124, 20), Filename);
			if (Filename != f)
			{
				m_SelectedItem = -1;
		...
		pos = m_Mode == Mode.Open ?
			new Vector2(Screen.width / 2 - 204, 0) : new Vector2(Screen.width / 2 - 120, 0);
		...
		// --- cancel button ---
		pos = m_Mode == Mode.Open ?
			new Vector2(Screen.width / 2 - 120, 0) : new Vector2(Screen.width / 2 - 120, 24);
